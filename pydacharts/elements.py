...
class PointsElements(BaseModel):
    ...
    hitRadius: Optional[number] = Field(None, description="Extra radius added to point radius for hit detection.")
    hoverRadius: Optional[number] = Field(None, description="Point radius when hovered.")
    hoverBorderWidth: Optional[number] = Field(None, description="Stroke width when hovered.")


class LinesElements(BaseModel):
    tension: Optional[number] = Field(None, description="Bézier curve tension (0 for no Bézier curves).")
    ...
    borderDashOffset: Optional[number] = Field(None, description="Line dash offset. See MDN")
    borderJoinStyle: Optional[Literal["round", "bevel", "miter"]] = Field(None, description="Line join style. See MDN")
    capBezierPoints: Optional[bool] = Field(None,
        description="true to keep Bézier control inside the chart, false for no restriction."
    )
    cubicInterpolationMode: Optional[str] = Field(None, description="Interpolation mode to apply. See more...")
    fill: Optional[Union[bool, str]] = Field(None, description="How to fill the area under the line. See area charts.")
    stepped: Optional[bool] = Field(None, description="true to show the line as a stepped line (tension will be ignored).")
...
class BarsElements(BaseModel):
    ...
    borderSkipped: Optional[Literal["start", "end", "middle", "bottom", "left", "top", "right", False]] = Field(None,
        description="Skipped (excluded) border: 'start', 'end', 'middle', 'bottom', 'left', 'top', 'right' or false."
    )
    borderRadius: Optional[Union[number, Dict]] = Field(None, description="The bar border radius (in pixels).")
    inflateAmount: Optional[Union[number, Literal["auto"]]] = Field(None,
        description="The amount of pixels to inflate the bar rectangle(s) when drawing."
    )
    pointStyle: Optional[PointStyle] = Field(None, description="Style of the point for legend.")
...
class ArcElements(BaseModel):
    ...
    borderAlign: Optional[Literal["center", "inner"]] = Field(None, description="Arc stroke alignment.")
    borderColor: Optional[Color] = Field(None, description="Arc stroke color.")
    borderJoinStyle: Optional[Literal["round", "bevel", "miter", "bevel", "round"]] = Field(None,
